...
async def single_chat(client, **kwargs):
    ...
    while True:
        try:
            r = await client.post(**kwargs, timeout=200)
            # r = await client.post(**kwargs, timeout=200,follow_redirects=True)
...
class ChatLM(BaseLM):
    ...
    def __init__(self, model, truncate=False):
        ...
        self.tokenizer = transformers.GPT2TokenizerFast.from_pretrained("GPT2_Path")
        self.headers = {
    ...
    def greedy_until(self, requests):
        ...
        for chunk, until in tqdm(
            ...
            responses = asyncio.run(oa_completion(
                ...
                model='gork-3',  # gpt-3.5-turbo, gork-3, ERNIE-3.5-8K
